...
`ccs setup update --explain <setting>` says what any setting means -- its default, its valid values, and the environment variable that sets it -- and with no setting named, all of them. A JSON file cannot carry explanations, so they live in the tool and this is how you read them; the text is the same the tool uses itself, so it cannot fall behind what it describes. It writes nothing and needs no config file to work.
